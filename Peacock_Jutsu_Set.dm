				...
				while(src)
					for(var/mob/M in src.loc)
						...
						if(M.FrozenBind!="ChakraBeastBind")
							M<<"Fique Apertando o SPACE para sair do Stun!"
						if(M.Struggle>=StruggleOutOf)
							M<<"Você quebrou o peacock stun!"
							M.FrozenBind=""
							del(src)
		...
		Bump(A)
			if(ismob(A))
				...
				if(!src.firing)
					...
					view(M)<<output("[M] Deu [damage] de dano com a besta de chakra!!!","Attack")
					src.loc=M.loc
			if(istype(A,/turf/))
				var/turf/T = A
				if(T.density)
	...
	KujakuMyouhou()
		...
		if(!src.KujakuMyouhouon)
			...
			if(startrainingstat<100)
				src<<"Você sai do primeiro estagio do Kujaku."
			if(startrainingstat>=100&&startrainingstat<300)
				src<<"Você sai do segundo estagio do Kujaku."
				return
			if(startrainingstat>=300&&startrainingstat<750)
				src<<"Você sai do terceiro estagio do Kujaku."
				return
			if(startrainingstat>=750&&startrainingstat<1000)
				src<<"Você sai do quarto estagio do Kujaku."
				return
			if(startrainingstat>=1000)
				src<<"Você sai do estagio final do Kujaku!"
				return
		else
			...
			src<<"Você desliga o Kujaku.";src.underlays -= 'Icons/Jutsus/Kujaku.dmi'
